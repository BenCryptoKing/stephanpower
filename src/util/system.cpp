...
const char * const BITCOIN_CONF_FILENAME = "stephanpower.conf";
...
#else
    const char* pszModule = "stephanpower";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stephanpower";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Stephanpower";
#else
    // Unix
    return pathRet / ".stephanpower";
#endif
#endif
}
