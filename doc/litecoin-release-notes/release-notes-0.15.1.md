Stephanpower Core version *0.15.1* is now available from:

  <https://download.stephanpower.org/stephanpower-0.15.1/>
...
A number of changes to the way Stephanpower Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 goblets per kB (0.00001 NSP). This is to prepare for dropping the minimum transaction fee to 0.1 goblets per kB (0.0001 NSP) in the next release.
